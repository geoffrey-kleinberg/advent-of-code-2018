...
with open('12-15-18/input.txt', 'r') as ifile:
...
def find_mov_target(field, unit, debug=False):
    start = (unit[1], unit[2])
    queue = deque([start])
    cseen = {}
    while True:
...
def sim_battle(field, units, elfpw):
    for turn in count():
        ...
        for uidx, unit in enumerate(units):
            debug = False
            if turn == 1 and unit[1] == 12 and unit[2] == 12:
                debug = True
            ...
            trgt = find_mov_target(field, unit, debug)
